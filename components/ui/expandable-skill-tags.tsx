...
export const ExpandableSkillTags = ({
  ...
  return (
    <section className={cn("w-full", className)}>
      <h3 className="mb-6 text-3xl font-bold tracking-tight md:text-4xl lg:text-5xl">{title}</h3>
      <motion.div
      ...
      >
        ...
        {visibleSkills.map((skill, index) => (
          <motion.div key={`visible-${index}`} variants={itemVariants}>
            <Badge variant="secondary" className="font-mono text-sm">{skill}</Badge>
          </motion.div>
        ))}
      ...
      {skills.length > initialCount && (
        <Button
          variant="link"
          className="mt-6 px-0 font-mono text-sm"
